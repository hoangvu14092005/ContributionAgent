...
class TestPRPatrolInit:
    ...
    async def test_get_user(self):
        github = MagicMock()
        github.get_authenticated_user = AsyncMock(return_value={"login": "chinhkrb113"})
        llm = MagicMock()
        patrol = PRPatrol(github=github, llm=llm)
        user = await patrol._get_user()
        assert user["login"] == "chinhkrb113"
        # Second call should use cache
        user2 = await patrol._get_user()
        assert user2["login"] == "chinhkrb113"
...
class TestCollectFeedback:
    ...
    async def test_filters_own_comments(self):
        ...
        github.get_pr_comments = AsyncMock(
            return_value=[
                {"id": 1, "user": {"login": "chinhkrb113", "type": "User"}, "body": "test"},
            ]
        )
        github.get_pr_review_comments = AsyncMock(return_value=[])
        llm = MagicMock()
        patrol = PRPatrol(github=github, llm=llm)
        result = await patrol._collect_feedback("owner", "repo", 1, "chinhkrb113")
        assert len(result) == 0
    ...
    async def test_bot_context_linked(self):
        ...
        llm = MagicMock()
        patrol = PRPatrol(github=github, llm=llm)
        result = await patrol._collect_feedback("owner", "repo", 1816, "chinhkrb113")
        assert len(result) == 1
...
class TestCheckAssignedIssues:
    ...
    async def test_finds_assigned_issues(self):
        ...
        result = PatrolResult()
        repos = {"owner/repo"}
        await patrol._check_assigned_issues(repos, "chinhkrb113", result, dry_run=True)
        assert result.issues_found == 1
    ...
    async def test_empty_repos(self):
        patrol = PRPatrol(github=MagicMock(), llm=MagicMock())
        result = PatrolResult()
        await patrol._check_assigned_issues(set(), "chinhkrb113", result)
        assert result.issues_found == 0
    ...
    async def test_handles_api_error(self):
        ...
        result = PatrolResult()
        await patrol._check_assigned_issues({"owner/repo"}, "chinhkrb113", result)
        assert result.issues_found == 0
