...
def adapt_pr_body(
    ...
    type_info = {
        ...
        ContributionType.REFACTOR: "Refactoring",
    }
    label = type_info.get(finding.type, "Fix")
    ...
    return _default_pr_body(contribution, label, files_list)
...
def _default_pr_body(
    ...
    return (
        ...
        f"- [x] Self-review of code completed\n"
        f"- [x] Changes generate no new warnings\n"
        f"- [ ] Corresponding changes to documentation made (if applicable)\n\n"
        f"**Severity**: `{finding.severity.value}`\n\n"
        f"\n\n"
