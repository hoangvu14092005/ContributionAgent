...
class CodeAnalyzer:
    ...
    def _security_prompt(self, ctx: RepoContext) -> str:
        ...
        return (
            ...
            "Return findings as JSON array in ```json``` code block:\n"
    ...
    def _ui_ux_prompt(self, ctx: RepoContext) -> str:
        ...
        return (
            ...
            "If no frontend code found, return empty array.\n\n"
            "Return findings as JSON array in ```json``` code block:\n"
            '[\n  {\n    "title": "Brief title",\n    "description": "Detailed description",\n'
        ...
        try:
            ...
            if json_match:
                try:
                    ...
                    return self._create_findings_from_items(items, contrib_type, analyzer_name)
            ...
            if "```yaml" in response:
                ...
                if len(parts) > 1:
                    yaml_parts = parts[1].split("```")
                    if len(yaml_parts) > 0:
                        yaml_text = yaml_parts[0]
            elif "```" in response:
            ...
            # Try safe_load with error handling
            try:
                ...
                if json_match:
                    try:
                        ...
                        logger.warning("JSON fallback also failed for %s", analyzer_name)
                        return []
                else:
                    ...
                    if json_match:
                        try:
                            ...
                            logger.warning("All parsing attempts failed for %s", analyzer_name)
                            return []
                    else:
                        return []
    ...
    def _create_findings_from_items(
        self, items: list, contrib_type: ContributionType, analyzer_name: str
    ) -> list[Finding]:
